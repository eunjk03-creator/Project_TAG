  ...
  return (
    <div className="min-w-0 flex flex-col">
      ...
      <div className="min-w-0 flex flex-col">
        ...
        {/* ── 이상치 부서별 현황 ── */}
        {openSections.has('anomaly') && (() => {
          // 부서별 이상치 집계
          const empById = new Map(baseEmployees.map(e => [e.id, e]))
          const divMap: Record<string, { late: number; early: number; shortWork: number; missing: number; compound: number; total: number }> = {}
          for (const r of scopedRecords) {
            if (approvedKeys.has(`${r.employeeId}_${r.date}`)) continue
            if (!r.flag) continue
            const div = empById.get(r.employeeId)?.division ?? '기타'
            if (!divMap[div]) divMap[div] = { late: 0, early: 0, shortWork: 0, missing: 0, compound: 0, total: 0 }
            const d = divMap[div]
            d.total++
            const f = r.flag
            if (f === 'LATE' || f === 'LATE_AND_EARLY_DEPARTURE' || f === 'LATE_AND_ANOMALY') d.late++
            if (f === 'EARLY_DEPARTURE' || f === 'LATE_AND_EARLY_DEPARTURE') d.early++
            if (f === 'ATTENDANCE_ANOMALY' || f === 'LATE_AND_ANOMALY') d.shortWork++
            if (f === 'NO_CLOCK_IN' || f === 'NO_CLOCK_OUT') d.missing++
            if (f === 'LATE_AND_EARLY_DEPARTURE' || f === 'LATE_AND_ANOMALY') d.compound++
          }
          const rows = Object.entries(divMap).sort((a, b) => b[1].total - a[1].total)
          const totals = rows.reduce((acc, [, d]) => ({
            late: acc.late + d.late, early: acc.early + d.early,
            shortWork: acc.shortWork + d.shortWork, missing: acc.missing + d.missing,
            compound: acc.compound + d.compound, total: acc.total + d.total,
          }), { late: 0, early: 0, shortWork: 0, missing: 0, compound: 0, total: 0 })
          return (
            <div className="px-6 pb-3 shrink-0">
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                <div className="px-4 py-2.5 border-b border-gray-100">
                  <p className="text-xs font-semibold text-gray-700">이상치 부서별 현황</p>
                </div>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-gray-50 text-gray-500 text-[11px]">
                        ...
                        <th className="text-center px-3 py-2 font-semibold text-amber-600">지각</th>
                        <th className="text-center px-3 py-2 font-semibold text-orange-500">조기퇴근</th>
                        <th className="text-center px-3 py-2 font-semibold text-red-600">근무시간미달</th>
                        <th className="text-center px-3 py-2 font-semibold text-red-500">미태깅</th>
                        <th className="text-center px-3 py-2 font-semibold text-purple-500">혼합</th>
                        <th className="text-center px-3 py-2 font-semibold text-gray-700">총합계</th>
                      </tr>
                    </thead>
                    <tbody>
                      {rows.map(([div, d]) => (
                        <tr key={div} className="border-t border-gray-50 hover:bg-gray-50/60">
                          <td className="px-3 py-2 font-medium text-gray-700 truncate max-w-[120px]">{div}</td>
                          <td className="px-3 py-2 text-center tabular-nums">{d.late > 0 ? <span className="text-amber-600 font-semibold">{d.late}</span> : <span className="text-gray-200">—</span>}</td>
                          <td className="px-3 py-2 text-center tabular-nums">{d.early > 0 ? <span className="text-orange-500 font-semibold">{d.early}</span> : <span className="text-gray-200">—</span>}</td>
                          <td className="px-3 py-2 text-center tabular-nums">{d.shortWork > 0 ? <span className="text-red-600 font-semibold">{d.shortWork}</span> : <span className="text-gray-200">—</span>}</td>
                          <td className="px-3 py-2 text-center tabular-nums">{d.missing > 0 ? <span className="text-red-500 font-semibold">{d.missing}</span> : <span className="text-gray-200">—</span>}</td>
                          <td className="px-3 py-2 text-center tabular-nums">{d.compound > 0 ? <span className="text-purple-500 font-semibold">{d.compound}</span> : <span className="text-gray-200">—</span>}</td>
                          <td className="px-3 py-2 text-center tabular-nums font-bold text-gray-800">{d.total}</td>
                        </tr>
                      ))}
                    </tbody>
                    <tfoot>
                      <tr className="border-t-2 border-gray-200 bg-gray-50 font-bold text-gray-700">
                        <td className="px-3 py-2">총합계</td>
                        <td className="px-3 py-2 text-center tabular-nums text-amber-600">{totals.late || '—'}</td>
                        <td className="px-3 py-2 text-center tabular-nums text-orange-500">{totals.early || '—'}</td>
                        <td className="px-3 py-2 text-center tabular-nums text-red-600">{totals.shortWork || '—'}</td>
                        <td className="px-3 py-2 text-center tabular-nums text-red-500">{totals.missing || '—'}</td>
                        <td className="px-3 py-2 text-center tabular-nums text-purple-500">{totals.compound || '—'}</td>
                        <td className="px-3 py-2 text-center tabular-nums text-gray-800">{totals.total}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
