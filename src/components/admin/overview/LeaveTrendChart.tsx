        ...
        <div className="h-[208px]">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={points} margin={{ top: 8, right: 8, left: -20 }}>
              ...
              <XAxis dataKey="label" tick={{ fontSize: 10, fill: '#8b8d94' }} />
              <YAxis domain={[0, 100]} tick={{ fontSize: 9.5, fill: '#b8bac0' }} tickFormatter={v => `${v}%`} width={34} />
              <Tooltip formatter={(v: unknown, name: unknown) => [`${Number(v ?? 0).toFixed(1)}%`, String(name ?? '')]} />
              {mode === 'cumulative' ? (
                <>
                  <Line dataKey="benchmarkPct" name="누적 목표" stroke="#2e2f33" strokeWidth={1.5} strokeDasharray="5 4" dot={false} />
                  <Line dataKey="cumulativePct" name="누적 실적" stroke="#e5342f" strokeWidth={2.5} dot={{ r: 3.5, fill: '#e5342f' }} connectNulls={false} />
                </>
              ) : (
                <>
                  <ReferenceLine y={MONTHLY_ALLOCATION} stroke="#2e2f33" strokeDasharray="6 4" strokeWidth={2}
                    label={{ value: `월 배분 목표 ${MONTHLY_ALLOCATION.toFixed(1)}%`, position: 'insideTopRight', fontSize: 10, fill: '#2e2f33' }} />
                  <Bar dataKey="singlePct" name="단월 사용률" radius={[5, 5, 0, 0]}>
                    {points.map((p, i) => (
                      <Cell key={i} fill={p.singlePct === null ? '#f1f2f4' : p.singlePct >= MONTHLY_ALLOCATION ? '#d17600' : '#e5342f'} />
                    ))}
                  </Bar>
                </>
              )}
