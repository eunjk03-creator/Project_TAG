...
export default function FastDashboard() {
  ...
  const overriddenRawRecords = useMemo(() => {
    ...
    return baseRecords.map(r => {
      ...
      if (!ov) return r
      return { ...r, clockIn: ov.clockIn, clockOut: ov.clockOut, erpOtApplied: ov.erpOtApplied !== null ? ov.erpOtApplied : r.erpOtApplied }
    })
  ...
  const allProcessed = useMemo<ProcessedRecord[]>(() => {
    ...
    return dateFiltered.map(r => {
      const ov = recordOverrides[`${r.employeeId}_${r.date}`]
      if (!ov) return r
      return processRecord({ ...r, clockIn: ov.clockIn ?? r.clockIn, clockOut: ov.clockOut ?? r.clockOut, erpOtApplied: ov.erpOtApplied !== null ? (ov.erpOtApplied as boolean) : r.erpOtApplied }, policy, otExemptIds, slackNoteMap, finalAttrMap.get(r.employeeId))
    })
