...
| B15 | ✅ 수정됨(2026-07-10, 커밋 c5e8cfa, d9539fd) | `record.leaveType`(ERP 원본과 Slack 주입 값이 한 필드에 섞여 있음)를 구분 없이 표시하는 곳이 2곳 있었음 — `DailyDetailModal.tsx`의 "ERP 연차·휴가" 패널(초록 상태점+실선 뱃지)과 `EmployeeCalendarGrid.tsx` 근태상태 행의 '연차' 태그(항상 실선). 둘 다 반차/반반차 콤보 뱃지엔 이미 있던 "ERP 미신청 시 점선+반투명" 처리가 연차엔 빠져있던 것. `verificationNote`의 'ERP 미신청' 마커로 분기해서 통일. 프론트엔드 표시 전용이라 캐시 재생성 불필요, 배포 즉시 반영 |
